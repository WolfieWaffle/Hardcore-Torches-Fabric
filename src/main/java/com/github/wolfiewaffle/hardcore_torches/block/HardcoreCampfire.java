...
public class HardcoreCampfire extends CampfireBlock implements IFuelBlock {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world.isClient) {
            return state.get(LIT) ? validateTicker(type, Mod.CAMPFIRE_BLOCK_ENTITY, HardcoreCampfireBlockEntity::clientTick) : null;
        } else {
            return state.get(LIT) ? validateTicker(type, Mod.CAMPFIRE_BLOCK_ENTITY, HardcoreCampfireBlockEntity::cookTick) : validateTicker(type, Mod.CAMPFIRE_BLOCK_ENTITY, HardcoreCampfireBlockEntity::cooldownTick);
        }
    }
