...
public class Mod implements ModInitializer {
	...
	public static final LootFunctionType HARDCORE_TORCH_LOOT_FUNCTION = new LootFunctionType(TorchLootFunction.CODEC);
	public static final LootFunctionType FUEL_LOOT_FUNCTION = new LootFunctionType(LanternLootFunction.CODEC);
	...
	public void onInitialize() {
		...
		LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
			...
			if (source.isBuiltin() && HOGLIN_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 3, 5);
			}

			if (source.isBuiltin() && SNIFFER_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 1, 3);
			}

			if (source.isBuiltin() && CAMEL_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 1, 3);
			}

			if (source.isBuiltin() && POLAR_BEAR_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 1, 3);
			}

			if (source.isBuiltin() && PANDA_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 1, 2);
			}

			if (source.isBuiltin() && PIG_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 1, 2);
			}

			if (source.isBuiltin() && PIGLIN_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 1, 2);
			}

			if (source.isBuiltin() && PIGLIN_BRUTE_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 1, 2);
			}

			if (source.isBuiltin() && COW_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 0, 2);
			}

			if (source.isBuiltin() && MOOSHROOM_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 0, 2);
			}

			if (source.isBuiltin() && SHEEP_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 0, 2);
			}

			if (source.isBuiltin() && LLAMA_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 0, 1);
			}

			if (source.isBuiltin() && TRADER_LLAMA_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 0, 1);
			}

			if (source.isBuiltin() && CHICKEN_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 0, 1);
			}
			...
			if (source.isBuiltin() && DONKEY_LOOT_TABLE_ID.equals(id)) {
				addFatLoot(tableBuilder, 0, 1);
			}
		});
	...
	private void addFatLoot(LootTable.Builder tableBuilder, int min, int max) {
		LootNumberProvider provider = new UniformLootNumberProvider(ConstantLootNumberProvider.create(min), ConstantLootNumberProvider.create(max));
		SetCountLootFunction func = (SetCountLootFunction) SetCountLootFunction.builder(provider).build();

		LootPool.Builder poolBuilder = LootPool.builder()
				.with(ItemEntry.builder(Mod.ANIMAL_FAT)).apply(func);
